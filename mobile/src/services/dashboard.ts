...
export async function getDashboardMetrics(): Promise<DashboardMetrics> {
  ...
  const [activeAlertsRes, openIncidentsRes, todayCheckinsRes, resolvedTodayRes] = await Promise.all([
    ...
    // Using created_at until an updated_at trigger/column is added.
    supabase.from('incidents').select('id', { count: 'exact', head: true }).eq('status', 'resolved').gte('created_at', `${date}T00:00:00Z`).lt('created_at', `${date}T23:59:59Z`),
  ]);
