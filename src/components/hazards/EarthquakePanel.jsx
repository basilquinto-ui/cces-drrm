...
export default function EarthquakePanel({ isAdmin = false }) {
  ...
  async function onSubmit(event) {
    ...
    const payload = {
      ...
      bulletin_url: formData.bulletin_url.trim() || null
    }
      ...
      {isAdmin ? (
        <form onSubmit={onSubmit} style={{ marginTop: 8, display: 'grid', gap: 8 }}>
          ...
          <div style={{ display: 'grid', gap: 8, gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))' }}>
            ...
            <input className="form-control" name="magnitude" placeholder="Magnitude" value={formData.magnitude} onChange={onChange} />
            <input className="form-control" name="depth_km" placeholder="Depth (km)" value={formData.depth_km} onChange={onChange} />
            <input className="form-control" name="latitude" placeholder="Latitude" value={formData.latitude} onChange={onChange} />
            <input className="form-control" name="longitude" placeholder="Longitude" value={formData.longitude} onChange={onChange} />
            <input className="form-control" name="intensity" placeholder="Intensity" value={formData.intensity} onChange={onChange} />
            <input className="form-control" name="distance_km" placeholder="Distance (km)" value={formData.distance_km} onChange={onChange} />
