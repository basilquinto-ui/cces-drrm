...
export default function Alerts({ isAdmin }) {
  ...
  async function loadAlerts() {
    ...
    setSuccess('')
    try {
      setAlerts(await fetchAlerts(filters))
    } catch (e) {
  ...
  async function handleCreate(e) {
    ...
    const issuedBy = data?.user?.email || 'System User'
  ...
  async function handleToggle(alert) {
    ...
    try {
      ...
      setSuccess(`Alert marked as ${alert.active ? 'resolved' : 'active'}.`)
      toast(`Alert marked as ${alert.active ? 'resolved' : 'active'}.`, 'success')
      loadAlerts()
    } catch (e) {
    ...
    <div className="fade-in">
        ...
        <section className="portal-card" style={{ marginTop: 14 }}>
          ...
          <form onSubmit={handleCreate}>
            <div className="form-group">
              ...
              <input className="form-control" value={form.hazard_type} onChange={e => setForm({ ...form, hazard_type: e.target.value })} placeholder="e.g. flood" required />
            </div>
            <div className="form-group">
              <label>Level</label>
      ...
      <section className="portal-card" style={{ marginTop: 14 }}>
        ...
        <div className="dashboard-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))' }}>
          ...
          <div className="form-group"><label>Hazard Type</label><select className="form-control" value={filters.hazardType} onChange={e => setFilters({ ...filters, hazardType: e.target.value })}>{hazardOptions.map(h => <option key={h} value={h}>{h === 'all' ? 'All' : h.replace(/_/g, ' ')}</option>)}</select></div>
      ...
      {!loading && !error && (
        <section className="portal-card" style={{ marginTop: 14 }}>
          {alerts.length === 0 ? <p>No alerts match the current filters.</p> : (
            <div className="monitor-table-wrap">
              <table className="monitor-table">
                <thead><tr><th>Created</th><th>Hazard Type</th><th>Level</th><th>Message</th><th>Issued By</th><th>Status</th>{isAdmin && <th>Action</th>}</tr></thead>
                <tbody>
                  {alerts.map(a => (
                    <tr key={a.id}>
                      <td>{new Date(a.created_at).toLocaleString()}</td>
                      <td>{(a.hazard_type || '-').replace(/_/g, ' ')}</td>
                      <td><span className="badge">{LEVEL_LABELS[a.level] || a.level}</span></td>
                      <td>{a.message}</td>
                      <td>{a.issued_by}</td>
                      <td><span className={`status-badge status-${a.active ? 'alert' : 'normal'}`}>{a.active ? 'Active' : 'Resolved'}</span></td>
                      {isAdmin && <td><button className="btn btn-outline btn-sm" onClick={() => handleToggle(a)}>{a.active ? 'Mark Resolved' : 'Mark Active'}</button></td>}
                    </tr>
                  ))}
                </tbody>
              </table>
