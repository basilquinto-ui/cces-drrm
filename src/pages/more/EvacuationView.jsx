...
export default function EvacuationView({ routes }) {
  return (
    <>
      <div className="section-title" style={{ marginBottom: 14 }}>Evacuation Routes</div>
      {routes.map((route) => (
        <div
          key={route.id}
        ...
        >
          <h4 style={{ fontSize: 14, fontWeight: 800, color: 'var(--navy)' }}>
            <span className="icon-badge">{HAZARD_ICONS[route.hazard_type] || 'HZ'}</span> {route.hazard_type.charAt(0).toUpperCase() + route.hazard_type.slice(1)}
          </h4>
          ...
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--green)', marginTop: 8 }}>Assembly Area: {route.assembly_area}</div>
          {route.map_url && (
            <a
              href={route.map_url}
              target="_blank"
