...
function getResourceIcon(name) {
  ...
  if (name.includes('Mega')) return 'MP'
  ...
  return (
    <>
      ...
      {Object.entries(grouped).map(([category, items]) => {
        const CategoryIcon = CAT_ICONS[category]
        return (
          <div key={category} className="card">
            <div className="card-title"><CategoryIcon size={16} /> {category.charAt(0).toUpperCase() + category.slice(1)}</div>
            {items.map((resource) => (
              <div key={resource.id} className="resource-item">
                ...
                <div className="resource-qty">
                  <div className="qty-num" style={{ color: resource.condition === 'replace' ? 'var(--red)' : resource.condition === 'low' ? 'var(--yellow)' : 'var(--navy)' }}>
                    {resource.quantity}
                  </div>
                  <span className={`badge badge-${resource.condition}`}>{resource.condition}</span>
                </div>
                {isAdmin && (
                  <button className="btn-icon" style={{ background: 'var(--blue-light)', color: '#3b82f6', marginLeft: 8 }} onClick={() => onEdit(resource)}>
                    <Pencil size={16} />
                  </button>
                )}
              </div>
