...
function AppInner() {
  ...
  useEffect(() => {
    if (tab === 'resources') fetchResources().then((r) => setData((v) => ({ ...v, resources: r.data || [] })))
    if (tab === 'routes') fetchRoutes().then((r) => setData((v) => ({ ...v, routes: r.data || [] })))
    if (tab === 'drills') fetchDrills().then((r) => setData((v) => ({ ...v, drills: r.data || [] })))
    if (tab === 'admin' && isAdmin) fetchStaff().then((r) => setData((v) => ({ ...v, staff: r.data || [] })))
  }, [tab, isAdmin])

  async function loadWeather() {
    const nextWeather = await fetchWeather()
    setWeather({ ...nextWeather, loading: false })
  }

  async function saveResource(form) {
    const current = data.editRes
    if (!current) return
    ...
    setData((v) => ({ ...v, editRes: null }))
    const refreshed = await fetchResources()
    setData((v) => ({ ...v, resources: refreshed.data || [] }))
  }
  ...
  const pages = {
    ...
    routes: <EvacuationRoutes routes={data.routes} />,
    drills: <Drills drills={data.drills} />,
  ...
  return (
    <AppShell active={tab} onNavigate={setTab} role={role} status={status} isAdmin={isAdmin}>
